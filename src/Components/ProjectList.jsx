...
const residentialImageModules = import.meta.glob('../assets/residential/*.jpg', { eager: true, as: 'url' });
const greenwaysImageModules = import.meta.glob('../assets/resgreenways/interior/*.{png,jpg}', { eager: true, as: 'url' });
const kanyakumariImageModules = import.meta.glob('../assets/kanyakumaribridge/*.jpg', { eager: true, as: 'url' });
...
const processModules = (modules) => {
    return Object.keys(modules)
        ...
        .map(path => ({
            src: modules[path],
            alt: path.split('/').pop().replace(/_/g, ' ').replace(/\.(png|jpg|jpeg)/gi, ''),
...
export default function ProjectList() {
  ...
  return (
    ...
    <div ref={projectListRef} className="flex flex-col gap-4 py-16 px-80 lg:px-[28rem]">
      {projectsData.map((project, index) => (
        <GalleryListItem
          ...
          scrollProgress={activeGalleryIndex === index ? activeItemScrollProgress : 0}
          ref={addToRefs}
        />
      ))}
    </div>
